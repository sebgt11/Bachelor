...
//table of binomial numbers shared by every bitvector
...
//set the bit at position i to one
...
//set the bit at position i to zero
...
//write len bits of val into W starting at position start
...
//read the bits from start to stop as one number
...
//count the one bits in x
static inline int popcount(uint64_t x)
{
    return __builtin_popcountll(x);
}

//fill the table of binomial numbers
...
//work out how many offset bits each class needs
...
//turn a block into its class and its offset
static void encode(uint64_t *W, uint64_t start, int b, uint64_t K[65][65], int *c_out, uint64_t *o_out)
{
    *c_out = 0;
    for (int i = 1; i <= b; i++)
    {
...
//turn a class and offset back into a block
...
//store the position of every one bit
...
//build the shared table once before any bitvector
...
//count the one bits up to position i
...
//return the position of the jth one bit
static int bitvector_select(Bitvector bv, int j)
{
    return (int)bv.S[j - 1];
}
